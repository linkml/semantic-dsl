...
SOURCE_SCHEMA_DIR = src/semdsl/datamodel
SRC = src
DEST = project
...
gen-project: $(PYMODEL)
	$(RUN) gen-project ${GEN_PARGS} -d $(DEST) $(SOURCE_SCHEMA_PATH)

MODULES = semdsl_model
gen-pydantic: $(SOURCE_SCHEMA_PATH) $(patsubst %, $(SOURCE_SCHEMA_DIR)/%.py, $(MODULES))

%.py: %.yaml  $(SOURCE_SCHEMA_DIR)
	$(RUN) gen-pydantic --no-mergeimports $< > $@

test: test-python test-doc

test-doc:
	$(RUN) python -m doctest --option ELLIPSIS --option NORMALIZE_WHITESPACE -v README.md src/semdsl/*.py src/semdsl/*/*.py
...
gen-examples: src/owl_datamodel/schema/owl_datamodel.yaml examples/output
	$(RUN) linkml-run-examples --no-use-type-designators \
		--output-formats json \
		--output-formats yaml \
		--counter-example-input-directory src/data/examples/invalid \
		--input-directory src/data/examples/valid \
		--output-directory examples/output \
		--schema $< > examples/output/README.md
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/owl_datamodel/schema/*yaml src/*/datamodel/*py src/data src/docs tests src/*/_version.py
